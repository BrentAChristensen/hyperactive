...
# script to follow an aruco marker with a robot arm using PyMoveit2.
...
class ArucoMarkerFollower(Node):
    ...
    def handle_aruco_markers(self, msg: ArucoMarkers):
        try:
            ...
            if self._prev_marker_pose is not None:
                if ((cal_marker_pose.position.x -
                    ...
                    (cal_marker_pose.position.z -
                     self._prev_marker_pose.position.z)**2 > 0.10**2):
            ...
            self.logger.error(f"Error handling aruco markers: {e}")
    ...
    def _transform_pose(self, pose: Pose, source_frame,
        ...
        try:
            ...
            self.logger.error(f"Error during pose transformation: {e}")
            raise

    def move_to(self, msg: Pose):
        try:
            initial_orientation = [msg.orientation.x, msg.orientation.y,msg.orientation.z,msg.orientation.w]

            rotation_quaternion = quaternion_from_euler(np.pi, 0, 0)
            new_orientation = quaternion_multiply(initial_orientation, rotation_quaternion)
            quat_xyzw = [new_orientation[0],new_orientation[1],new_orientation[2],new_orientation[3]]
            ...
            if not self._lock.acquire(blocking=False):
                self.logger.info("Process lock set")
                return  # Skip execution if another instance is running
            self.moveit2.move_to_pose(pose=pose_goal)
            self.moveit2.wait_until_executed()
        except Exception as e:
            self.logger.error(f"Error during move command: {e}")
        finally:
            self._lock.release()
